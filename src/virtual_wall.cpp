    ...
    */
    void VirtualWall::updateCosts(costmap_2d::Costmap2D& master_grid, int min_i, int min_j, int max_i, int max_j)
    {
        ...
        if (!virtualWall_received_)
            return;
        ros::Time  last_time= ros::Time::now();                                    // 获取当前时刻
	// cout << "dt = :" << dt << endl;
        // updateWithMax(master_grid, min_i, min_j, max_i, max_j);
        updateWithTrueOverwrite(master_grid, min_i, min_j, max_i, max_j);
        // ros::Time   current_time = ros::Time::now();    // 获取当前时刻
        
        // int dt = (current_time - last_time).toSec();// 计算里程间隔时间
        // cout<<"dt="<<dt <<endl;                   
...
// cout << "point.y" <<point.y<< endl;
         ...
         wallmin_y = std::min(wallmin_y, point.y);
          ...
          bool ret = worldToMap(point.x, point.y, pixle_x, pixle_y);                        
          if (ret)
          {
            // cout << " (" << pixle_x << ", " << pixle_y << ") "<<endl;
            //if (wallPoint[i].x<min_i||wallPoint[i].x>max_i||wallPoint[i].y<min_j||wallPoint[i].y>max_j) continue;
            //master->setCost(pixle_x, pixle_y, costmap_2d::LETHAL_OBSTACLE);
              unsigned int index = getIndex(pixle_x, pixle_y);
